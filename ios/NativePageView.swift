...
//          maybeScrollView!.keyboardDismissMode = _dismissKeyboard;
  ...
  override func layoutSubviews() {
    ...
    self.sortedCachedViewControllers[self.initialPage].view.frame = bounds
...
extension NativePageView: UIScrollViewDelegate {
  func scrollViewDidScroll(_ scrollView: UIScrollView) {
    ...
    let offset = offsetPercent.truncatingRemainder(dividingBy: 1);
    if (offset == 0) {
      return;
    }
#if DEBUG
...
#endif
    self.eventDispatcher!.send(RCTOnPageScrollEvent(reactTag: self.reactTag, position: NSNumber(value: leftIndex), offset: NSNumber(value: offset)));
  }
